        ...
        if 'Collisions' == 'Collisions':
            ...
            for i in range(3):
                for j in range(i+1,3):                       
                    relative_velocity = ballsvectors[j] - ballsvectors[i]
                    distance = ti.math.sqrt((balls[i][0] - balls[j][0]) ** 2 + (balls[i][2] - balls[j][2]) ** 2)

                    normal = (balls[j] - balls[i]).normalized()
                    relative_speed = relative_velocity.dot(normal)
                    
                    if distance<=0.2:  
                        impulse = relative_speed * normal
                        ballsvectors[i] += impulse
                        ballsvectors[j] -= impulse
